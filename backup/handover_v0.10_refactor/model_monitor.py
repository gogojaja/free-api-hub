...
DEFAULT_LEDGER_DIR = "台账"
DEFAULT_KEY_DIR = os.path.join(os.path.expanduser("~"), ".config", "opencode")

LEDGERS = {k: os.path.join(DEFAULT_LEDGER_DIR, v) for k, v in LEDGER_FILES.items()}
KEY_DIR = DEFAULT_KEY_DIR


def ledger_paths(ledger_dir: str | None = None) -> dict:
    """返回台账文件完整路径字典。ledger_dir 为 None 时用 DEFAULT_LEDGER_DIR。"""
    base = ledger_dir or DEFAULT_LEDGER_DIR
    return {k: os.path.join(base, v) for k, v in LEDGER_FILES.items()}
...
def merge_quota_ledger(new_rows, ledger_dir: str | None = None):
    """27 台账 upsert：按 (平台,模型) 去重更新，保留人工备注行（非探针行）。"""
    path = ledger_paths(ledger_dir)["quota"]
...
def main():
    ...
    ap.add_argument("--ledger-dir", default=None,
                    help=f"台账目录（默认 {DEFAULT_LEDGER_DIR}/）")
    ap.add_argument("--key-dir", default=None,
                    help=f"API key 目录（默认 {DEFAULT_KEY_DIR}）")
    args = ap.parse_args()

    logging.basicConfig(level=logging.DEBUG if args.debug else logging.INFO)

    paths = ledger_paths(args.ledger_dir)
    ...
    if os.path.exists(paths["price"]):
        with open(paths["price"], encoding="utf-8-sig") as f:
    ...
    if args.write:
        with open(paths["price"], "w", encoding="utf-8-sig", newline="") as f:
            w = csv.DictWriter(f, fieldnames=list(merged[0].keys()))
            w.writeheader()
            w.writerows(merged)
        logger.info("已写入 %s（%d 行）", paths["price"], len(merged))

    # 免费额度核验（27 台账）：国产平台最小探测
    logger.info("开始国产平台免费额度核验（%d 平台）", len(QUOTA_PROBES))
    qrows = snapshot_quota(key_dir=args.key_dir)
    for r in qrows:
        logger.info("  核验 %s | %s | %s", r[0], r[1][:40], r[6][:50])
    if args.write:
        n = merge_quota_ledger(qrows, ledger_dir=args.ledger_dir)
        logger.info("免费额度核验结果已追加 %d 行 → %s", n, paths["quota"])
    return 0
