...
├── tests/
│   ├── test_regression.py # 集成回归测试（17 个用例，本地 5080）
│   ├── run_all_tests.py   # 统一测试调度入口（8 套件 41 用例 + CSV 汇总）
│   └── test_*.py          # M3 变更项单元测试（7 文件 24 用例）
├── docs/
│   └── design.md          # 设计文档
├── backup/                # 配置文件自动备份
...
| v3.7 | **M5 投产（Go-Live 评审 + 运维交接）**：Go-Live 评审 PASS（需求 17 全✅ / 变更 7 落地 / 测试 41 用例 / 安全门禁通过），产出 `评审报告_投产评审_v1.0.csv`；launchd 守护因 macOS TCC 外部卷权限（EX_CONFIG 78）阻塞，回滚至手动进程，风险登记 RISK-004；新增 `docs/运维手册.md` |
